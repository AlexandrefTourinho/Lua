...
local joints = {} -- Tabela para armazemaneto de corpos 
...
-- shape2.x, shape2.y, shape2.rotation = display.
-- contentCenterX+60, 170, 0
...
-- 					joints[#joints+1]= physics.newJoint ("friction", event.other, self, self.x, self.y) --> altera o valor maximo de força e torque de acordo com o objeto da if acima

				
-- 					joints[#joints].maxForce = forceLimit
-- 					joints[#joints].maxTorque = torqueLimit
-- 				end -- fecha a função do timer
-- 				)--> fecha o timer
...
-- --> criação das variaveis para armazenamento dos parametros de frequencia e dampingRatio
-- local springFrequency = 30
-- local springDampingRatio = 1.0
-- --> crianção da junta dentro da string joints ([#joints+1]) faz com qie a variavel nao sobre escreva 
-- joints[#joints+1] = physics.newJoint( "wheel", vehicleBody, wheelA, wheelA.x, wheelA.y, 1, 1 )
-- --> atribui a frequencia e damping a junta de valores ja criados anteriormente 
...
-- joints[#joints].springFrequency = springFrequency
-- joints[#joints].springDampingRatio = springDampingRatio
-- --> Aplicção de troque as rodas para se movimentar 
-- wheelA:applyTorque (2)
-- wheelA:applyTorque (2)
-- wheelA:applyTorque (2)

--------------------
local bodyA = display.newCircle (0, 0, 40)
bodyA:setFillColor (1, 2.0, 0.4)
physics.addBody (bodyA, "dynamic", {bounce=0.8,radius=40})
bodyA.x,bodyA.y = display.contentCenterX-50, 260
bodies[#bodies+1] = bodyA

local bodyB = display.newCircle (0, 0, 26)
bodyA:setFillColor (1, 2.0, 0.4)
...
--> criando a junta ("tipo de  junta", corpoA, corpoB, pino.x, pinoA.y, pinoB.y, ancoraA.x, ancoraA.y )
local joint = physics.newJoint ("pulley", bodyA, bodyB, bodyA.x, bodyB.y-100, bodyB.x, bodyB.y-140, bodyA.x, bodyA.y, bodyB.x, bodyB.y, 1.0)
