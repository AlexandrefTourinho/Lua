...
local fundo = display.newImageRect(backGroup,"imagens/fundo.jpg", 1024, 1024) --> (fundo é a variavel) display.newImageRect (usado pra chamar a imagem)
fundo.x = display.contentCenterX
fundo.y = display.contentCenterY


local circulo = display.newCircle  ( 530, 60, 60 )
circulo:setFillColor (0.5,0.9)
circulo.x = 150
circulo.y = 250
physics.addBody (circulo, {bounce=0.8, radius=48})
circulo.myName = "Bola um"

local baixo = display.newRect (display.contentCenterX, 480, 80, 50)
physics.addBody (baixo, "")
baixo.myName = "chao"
baixo:setFillColor (0.5,0.9)
...
local function colisaobaixo (event)
...
Circulo:addEventListener ("collision", colisaoCirculo)
local cor = 0
    local function colisaoCirculo (event)


        if (event.phase == "began") then
            print ("player colidiu! chão")


        elseif (event.phase == "ended") then
            if (cor == 0) then
            baixo:setFillColor (0, 1, 0)
            print ("fim da colisão")
            cor = 1
            elseif (cor == 1) then
                baixo:setFillColor (0, 0, 1)
                print ("fim da colisão")
                cor = 0
            end
        end
    ...
    baixo:addEventListener ("collision", colisaoChao)